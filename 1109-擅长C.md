...
```python
# 实现原理：
# 本题的核心是：解析句子中的单词，提取大写字母并映射到七段字形输出。程序先读取题目输入，再按照上述规则完成数据处理，最后严格按照题目要求输出结果。实现时应优先根据题目约束
...
g = [lines[i * 7:(i + 1) * 7] for i in range(26)]
line = lines[-1]
out = []
i = 0
while i < len(line):
    while i < len(line) and (not line[i].isupper()):
        i += 1
    j = i
    ...
    if j > i:
        if out:
            out.append('')
        out += [' '.join((g[ord(c) - 65][r] for c in line[i:j])) for r in range(7)]
    i = j
print('\n'.join(out))
```
