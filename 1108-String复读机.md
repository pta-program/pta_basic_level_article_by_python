...
```python
# 实现原理：
# 本题的核心是：统计 `String` 六个字符的数量，并按 `String` 的顺序循环输出仍有剩余的字符。程序先读取题目输入，再按照上述规则完成数据处理，最后严格按照
# 题目要求输出结果。实现时应优先根据题目约束选择合适的数据类型和数据结构，避免溢出或不必要的重复计算。
...
# 读取输入并执行题目要求的算法。
import sys
s = sys.stdin.buffer.read().decode().split()[0]
chars = 'String'
c = [s.count(x) for x in chars]
out = []
while any(c):
